...
import { convertDealToAdmissionSchema } from "@/lib/validations/deal.schema";
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const input = convertDealToAdmissionSchema.parse(body);
    ...
    const result = await DealService.convertToAdmission(ctx, id, input);
    return ok(result);
  } catch (err) {
