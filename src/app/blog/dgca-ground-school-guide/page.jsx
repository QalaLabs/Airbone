      ...
      <main style={{ minHeight: '80vh', background: 'var(--paper)', padding: '4rem var(--margin) 6rem var(--margin)' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <section style={{ marginBottom: '3rem' }}>
            <h2 style={{ fontFamily: 'var(--font-h)', fontSize: '1.2rem', fontWeight: 800, color: 'var(--navy)', textTransform: 'uppercase', marginBottom: '1.25rem' }}>DGCA Exam Schedule & Structure</h2>
            <div style={{ background: '#ffffff', border: '1px solid rgba(0,39,76,0.08)', padding: '1.75rem', borderRadius: '1px', marginBottom: '1.25rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.5rem' }}>
                {[
                  ...
                  <div key={item.label} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '0.72rem', color: 'rgba(33,33,33,0.55)', fontFamily: 'var(--font-h)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{item.label}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              {SUBJECTS.map((s, i) => (
                <div key={i} style={{ background: '#ffffff', border: '1px solid rgba(0,39,76,0.08)', borderLeft: `3px solid ${s.difficulty === 'Hard' ? '#DB241E' : s.difficulty === 'Medium' ? '#D8A027' : 'rgba(33,33,33,0.3)'}`, padding: '1.5rem', borderRadius: '1px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem', gap: '1rem' }}>
                    <h3 style={{ fontFamily: 'var(--font-h)', fontSize: '1rem', fontWeight: 800, color: 'var(--navy)', textTransform: 'uppercase', margin: 0 }}>{s.name}</h3>
                    <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
                      <span style={{ fontSize: '0.6rem', fontFamily: 'var(--font-h)', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', padding: '0.2rem 0.5rem', background: s.difficulty === 'Hard' ? 'rgba(219,36,30,0.15)' : s.difficulty === 'Medium' ? 'rgba(216,160,39,0.15)' : 'rgba(0,39,76,0.05)', color: s.difficulty === 'Hard' ? '#DB241E' : s.difficulty === 'Medium' ? '#D8A027' : 'rgba(33,33,33,0.5)', borderRadius: '2px' }}>{s.difficulty}</span>
                    </div>
                  </div>
                  <p style={{ fontSize: '0.85rem', color: 'rgba(33,33,33,0.7)', fontFamily: 'var(--font-b)', lineHeight: '1.6', margin: 0 }}>{s.desc}</p>
                </div>
              ))}
            </div>
          </section>
